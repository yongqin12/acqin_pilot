...
{
    // Convert from centidegrees on public interface to radians转化为弧度
    float euler_roll_angle = radians(euler_roll_angle_cd * 0.01f);
    float euler_pitch_angle = radians(euler_pitch_angle_cd * 0.01f);
    float euler_yaw_rate = radians(euler_yaw_rate_cds * 0.01f);

    // calculate the attitude target euler angles计算搜需要的目标姿态欧拉角（）加速度，获取当前姿态角
    _attitude_target_quat.to_euler(_attitude_target_euler_angle.x, _attitude_target_euler_angle.y, _attitude_target_euler_angle.z);
    ...
    if (_rate_bf_ff_enabled) {
        ...
        // When acceleration limiting and feedforward are enabled, the sqrt controller is used to compute an euler
        // angular velocity that will cause the euler angle to smoothly stop at the input angle with limited deceleration
        // and an exponential decay specified by smoothing_gain at the end.坐标变化
    ...
    // Call quaternion attitude controller呼叫四元数姿态控制器
    attitude_controller_run_quat();
}

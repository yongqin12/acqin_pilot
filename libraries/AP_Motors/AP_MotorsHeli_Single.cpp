...
bool AP_MotorsHeli_Single::init_outputs()
{
    ...
    //SRV_Channels::set_angle(SRV_Channel::k_motor3, YAW_SERVO_MAX_ANGLE);
...
uint16_t AP_MotorsHeli_Single::get_motor_mask()
{
    ...
    uint32_t mask = 1U << 0 | 1U << 1 | 1U << 2 | 1U << 3 | 1U << 4 |1U << AP_MOTORS_HELI_SINGLE_RSC;
...
void AP_MotorsHeli_Single::move_actuators(float roll_out, float pitch_out, float coll_in, float yaw_out)
{
    ...
    move_yaw(yaw_out + yaw_offset,pitch_out);
}

// move_yaw
void AP_MotorsHeli_Single::move_yaw(float yaw_out,float pitch_out)
{
    ...
    switch (_spool_state) {
        ...
        case SpoolState::THROTTLE_UNLIMITED:
            ...
            if (_tail_type == AP_MOTORS_HELI_SINGLE_TAILTYPE_DIRECTDRIVE_FIXEDPITCH){
                ...
                rc_write_angle(AP_MOTORS_MOT_4, _servo4_out * YAW_SERVO_MAX_ANGLE);
                rc_write_angle(AP_MOTORS_MOT_5, _servo5_out * YAW_SERVO_MAX_ANGLE);
                //rc_write_angle(AP_MOTORS_MOT_6, _servo5_out * YAW_SERVO_MAX_ANGLE);
            }
            break;

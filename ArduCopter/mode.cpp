...
bool Copter::set_mode(control_mode_t mode, mode_reason_t reason)
{
    ...
    if (!ignore_checks && !new_flightmode->has_manual_throttle() && (motors->get_spool_state() == AP_Motors::SpoolState::SPOOLING_UP || motors->get_spool_state() == AP_Motors::SpoolState::SPOOLING_DOWN)) {
        gcs().send_text(MAV_SEVERITY_WARNING,"Flight mode change failed,rotor runup is not complete");
        AP::logger().Write_Error(LogErrorSubsystem::FLIGHT_MODE, LogErrorCode(mode));
        return false;
    }
#endif
    ...
    if (!new_flightmode->init(ignore_checks)) {//进行模式的初始化
        gcs().send_text(MAV_SEVERITY_WARNING,"Flight mode change failed,Flight mode init failed");
        AP::logger().Write_Error(LogErrorSubsystem::FLIGHT_MODE, LogErrorCode(mode));
        return false;
    }
